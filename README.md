...
每只股票都有独立的后台参考价，每秒小幅随机游走。Bot 围绕参考价通过正常订单与撮合引擎提供流动性；用户仍可提交任意合法的正数限价。页面最新价只来自真实成交，Bot 不会主动追成交明显偏离参考价的极端挂单。
### Task 6D 模拟市场

后台参考价每秒变化；Bot 对三只股票每秒随机提交 2~4 笔 BUY 和 2~4 笔 SELL，价格约在参考价 ±0.5%，全部经过统一撮合引擎。未成交 Bot 订单约 9 秒自动撤销，用户订单可通过“取消”按钮撤销剩余部分。页面展示真实订单簿最优 5 档，成交价使用参考价在双方限价区间内的 clamp 值。
### Task 6E 交易信息展示
