...
describe('market simulator', () => {
  ...
  it('samples current latest prices without changing them', () => {
    ...
    expect(stock.latestPrice).toBe(128.52); expect(points['600519'].price).toBe(128.52); expect(stock.changePercent).toBe(Number((((128.52 - stock.initialPrice) / stock.initialPrice) * 100).toFixed(2))); expect(store.priceHistory.get('600519')).toHaveLength(beforeHistory + 1); expect(store.priceHistory.get('600519')?.at(-1)).toEqual(points['600519'])
  })

  it('keeps only the latest 90 sampled points', () => {
    const store = new MemoryStore()
    for (let index = 0; index < 100; index++) advanceMarket(store, () => 0.5)
    const history = store.priceHistory.get('600519')!
    expect(history).toHaveLength(90); expect(history.at(-1)?.price).toBe(store.stocks.get('600519')!.latestPrice)
  })
})
