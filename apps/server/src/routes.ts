...
export const createRoutes = (store: MemoryStore, onOrderResult?: (result: any) => void) => {
  ...
  router.post('/orders/:orderId/cancel', (req, res) => { try { const order = cancelOrder(store, String(req.body.userId), req.params.orderId); onOrderResult?.({ order, trades: [] }); res.json({ order }) } catch (error) { res.status(400).json({ error: (error as Error).message }) } })
  return router
}
