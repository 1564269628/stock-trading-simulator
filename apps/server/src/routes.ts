...
export const createRoutes = (store: MemoryStore) => {
  const router = Router()
  ...
  router.post('/orders', (req, res) => { try { res.status(201).json(submitOrder(store, req.body)) } catch (error) { res.status(400).json({ error: (error as Error).message }) } })
  return router
}
