...
function executionSummary(order: Order) { const related = currentMyTrades.value.filter(trade => tradeSide(trade) === order.side); const filledQuantity = related.reduce((sum, trade) => sum + trade.quantity, 0); const executionAmount = related.reduce((sum, trade) => sum + trade.price * trade.quantity, 0); return { filledQuantity, executionAmount, averageExecutionPrice: filledQuantity ? executionAmount / filledQuantity : 0 } }
