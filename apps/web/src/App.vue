...
async function cancel(order: Order) { try { await api(`/orders/${order.id}/cancel`, { method: 'POST', body: JSON.stringify({ userId: user.value?.userId }) }); await refreshState() } catch (error) { message.value = (error as Error).message } }
</script>
<template>
<main class="terminal">
