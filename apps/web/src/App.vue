  ...
  <main class="terminal">
    ...
    <template v-else>
      ...
      <section class="layout"><aside class="panel stock-list"><h3>自选股票</h3><button v-for="stock in state?.stocks" :key="stock.symbol" :class="{ active: stock.symbol === selectedSymbol }" @click="selectStock(stock)"><div class="stock-name">{{ stock.name }}</div><div>{{ stock.symbol }} · {{ stock.latestPrice.toFixed(2) }}</div><div :class="stock.changePercent >= 0 ? 'up' : 'down'">{{ stock.changePercent.toFixed(2) }}%</div></button></aside>
        <section class="panel"><div class="muted">最新价 / 涨跌幅</div><div class="price">{{ selectedStock?.latestPrice.toFixed(2) }} <span :class="(selectedStock?.changePercent ?? 0) >= 0 ? 'up' : 'down'">{{ selectedStock?.changePercent.toFixed(2) }}%</span></div><div class="panel muted">价格走势将在 Task 4 接入</div><div class="forms"><form class="trade-form buy" @submit.prevent="submit('BUY')"><h3>BUY 买入</h3><input v-model="buyPrice" type="number" min="0" step="0.01"><input v-model="buyQuantity" type="number" min="1"><button>提交买入</button></form><form class="trade-form sell" @submit.prevent="submit('SELL')"><h3>SELL 卖出</h3><p class="muted">持仓：{{ currentPosition }} · 可卖提示将在 Task 2 接入</p><input v-model="sellPrice" type="number" min="0" step="0.01"><input v-model="sellQuantity" type="number" min="1"><button>提交卖出</button></form></div><p class="error">{{ message }}</p></section>
        <aside class="panel"><h3>盘口</h3><p class="muted">盘口将在 Task 5 接入</p></aside>
      </section>
