...
> 当前状态：Implementation Plan（执行中）。  
> 实现分支：`feat/mvp`  
> Implementation PR：关联 Research PR #1，单个 PR 完成 MVP。  
> Research 分支 `research/initial-design` 保持不变，不合并、不在其上实现。

## 0. 执行边界与阶段门

本计划是 Implementation PR 的唯一执行边界。每个阶段完成后必须：

1. 只修改当前阶段需要的文件；
2. 运行当前阶段列出的验证命令；
3. 记录失败根因后做最小修复，再重新验证；
4. 通过后再进入下一阶段。

核心撮合采用 TDD：Task 3 必须先提交/运行失败测试，Task 4 才能实现撮合代码。除题目明确要求外，不增加数据库、Redis、消息队列、微服务、市场单、撤单或复杂风控。

### 阶段与验收门

| 阶段 | Tasks | 通过条件 |
|---|---:|---|
| 骨架与模型 | 1–2 | 根目录依赖安装、前后端可启动，模型和内存 Store 可编译 |
| 撮合核心 | 3–4 | 撮合测试先红后绿，覆盖价格/时间优先、部分成交和状态 |
| 账户与 API | 5–6 | 记账测试和 REST smoke test 通过 |
| 实时与界面 | 7–8 | WebSocket 事件、行情循环和 Vue 主流程可用 |
| 联调与交付 | 9–10 | `npm test`、`npm run build` 通过，README 可复现 |

### 完成定义

Implementation PR 只有在所有阶段门通过、需求验收清单完成、根目录 `npm install` 与 `npm run dev` 可复现，并完成独立 reviewer 审查后才视为完成。
